...
def validate_token(func):
    ...
    def wrap(self, *args, **kwargs):
        ...
        token = access_token_data.find_one_or_create_token(
            user_id=access_token_data.user_id.id)
        if not token or (token and token.token != access_token):
            return invalid_response(
...
def generate_api_log(func):
    """ Create a wrapper class to generate REST API log """

    @functools.wraps(func)
    def wrap(self, *args, **kwargs):
        """ Get the requests' details and create a log record """
        result = func(self, *args, **kwargs)
        request.env['rest.api.log'].sudo().create({
            'request_method': str(request.httprequest.method),
            'request_url': str(request.httprequest.url),
            'request_headers': str(request.httprequest.headers),
            'request_params': str(request.httprequest.args),
            'request_response': str(result.data),
            'request_uid': request.session.uid,
            'request_status': str(result.status_code),
